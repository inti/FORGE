...
if (is.null(snp_annot) ==FALSE){
    ...
    snp<-getBM(c("refsnp_id","chr_name","chrom_start", "chrom_strand","mapweight","allele"), filters="refsnp",values=colnames(genotypes),mart = snpmart)
}
...
assoc_file<-"~/DATA/WTCCC/WEB/basic/WTCCC_CD_imputed.txt_plusQC.filtered.pvals"
cat("Reading SNP p-values from [ ",assoc_file," ]\n",sep="")
assoc<-read.table(assoc_file)
if(ncol(assoc) < 2){
...
snp<-merge(snp,assoc,by.x="refsnp_id",by.y="SNP")
...
for(chr in all_chrs){ 
    ...
    if ( nrow(snp_chr) == 0){ 
        #cat("No SNPs mapped on chromsome [ ",chr," ]\n",sep="")
        next
    }
    ...
    gene_range<-IRanges(start=gene_chr$start_position,end=gene_chr$end_position)
    ...
    for ( my_gene_index in unique(map[,1])){
        report_advance(counter,report,"Genes")
        report<-report+1
        ...
        if ( nrow(gene_snps) == 0){ 
            ...
            cat(nrow(gene_snps)," SNPs mapped to gene [ ",gene_data$ensembl_gene_id," ]\n",sep="")  
        }
        if (nrow(gene_snps) == 1){
